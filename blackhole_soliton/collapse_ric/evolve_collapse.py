...
def main():
    ...
    # Final-time radial slices
    plt.figure()
    plt.plot(r, alpha[-1, :], label='Redshift α(r)', color='orange')
    plt.plot(r, R[-1, :], label='Ricci Scalar R(r)', color='red')
    plt.plot(r, phi[-1, :], label='Scalar Field φ(r)', color='blue')
    plt.xlabel("Radial coordinate r")
    plt.ylabel("Field values at t = t_final")
    plt.title("Final-State PWARI-G Field Profiles")
    plt.legend()
    plt.grid(True)
    plt.savefig("output/final_radial_profiles.png")
    plt.close()

    # Save Ricci heatmap
    plt.figure(figsize=(8, 6))
    extent = [r[0], r[-1], 0, nt * dt]
    plt.imshow(R[:nt, :], extent=extent, origin='lower', aspect='auto', cmap='plasma')
    plt.colorbar(label='Ricci Scalar R(r, t)')
    plt.xlabel('Radial coordinate r')
    plt.ylabel('Time')
    plt.title('PWARI-G Ricci Scalar Field R(r, t)')
    plt.savefig("output/ricci_heatmap.png")
    plt.close()
    ...
    # Save redshift heatmap with overlay
    plt.figure(figsize=(8, 6))
