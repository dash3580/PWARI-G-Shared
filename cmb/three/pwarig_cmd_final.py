...
for step in range(STEPS):
    ...
    # --- Enhanced Twist–Photon Coupling ---
    phi_ddot = laplacian(phi) - dV_dphi(phi) + 0.3 * np.sin(theta) + 0.2 * A
    theta_ddot = laplacian(theta) + 2 * (phi_dot * theta_dot + phi * laplacian(theta)) / np.clip(phi**2, 1e-2, 1e2) - 0.05 * theta_dot
    A_ddot = laplacian(A) - 0.05 * A + 0.2 * np.sin(theta)
...
if step == FREEZE_STEP:
    print(">>> Recombination Freeze-Out Activated at Step 1000 <<<")
    A_dot[:] = 0.0
    theta_dot[:] = 0.0
    freeze_A = A.copy()
    freeze_theta = theta.copy()

if step > FREEZE_STEP and freeze_A is not None and freeze_theta is not None:
    A = freeze_A
    theta = freeze_theta
